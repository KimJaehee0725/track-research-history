...
상태 변경 요청은 정확한 동일 출처 `Origin` 또는 `Referer`가 있어야 합니다. 이는
악성 웹페이지가 브라우저를 통해 loopback UI에 form POST를 보내는 것을 차단합니다.
브라우저에서 연 정상 UI 폼은 자동으로 필요한 출처 헤더를 보냅니다. `curl` 같은
진단 도구로 POST를 재현할 때는 현재 터널 URL과 일치하는 `Origin`을 명시해야
하며, 출처 검사를 비활성화하지 않습니다.
...
| 저장 요청이 403 | 오래된 탭이나 다른 hostname/port를 섞지 않았는지 확인하고 현재 loopback URL에서 페이지를 다시 열기 |
