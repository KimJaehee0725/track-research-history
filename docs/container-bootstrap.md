...
각 노트북 또는 연구실 서버에서 예제 파일을 저장소 밖으로 복사합니다.
...
`host`, `user`, `identity_file`, `known_hosts`를 실제 값으로 바꿉니다. 설정 파일에는 키 **경로**만 넣고 키 본문, passphrase, 토큰을 넣지 않습니다. 환경 변수를 선호하면 [examples/memory-client.env.example](../examples/memory-client.env.example)를 참고합니다.

정상 연결을 먼저 호스트에서 확인합니다.

```bash
client/memctl.py --config ~/.config/research-memory/client.json project list
client/memctl.py --config ~/.config/research-memory/client.json note list alienlm
```

`--config` 같은 연결 옵션은 명령의 앞에 둡니다. 일반 응답은 JSON이며, 자동화에서 결과 값만 필요하면 `--result-only`를 붙입니다.
...
client/memctl.py --config ~/.config/research-memory/client.json \
  note write alienlm notes/experiment-01.md --file ./experiment-01.md

# 표준입력도 가능합니다.
printf '# Meeting\n\n- next: run baseline\n' | \
  client/memctl.py --config ~/.config/research-memory/client.json \
  note write alienlm notes/meeting-2026-07-21.md --stdin
...
client/memctl.py --config ~/.config/research-memory/client.json \
  note search alienlm "recovery attacker budget" --limit 10
```
...
client/memory-run \
  ...
  --config "$HOME/.config/research-memory/client.json" \
...
python "$RESEARCH_MEMORY_MEMCTL" note search "$RESEARCH_MEMORY_PROJECT" "ablation"
```
